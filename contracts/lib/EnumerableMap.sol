 ...
 * supported.
 * 
 * NOTICE: Pulled from OZ library. All methods changed to internal so that additional
 * types can be extended. If the UintToAddressMap type is required, create a new
 * library and copy the implementation from OZ. It was deleted from this library for
...
library EnumerableMap {
    ...
    */
     ...
     */
    function _tryGet(Map storage map, bytes32 key) internal view returns (bool, bytes32) {
     ...
     */
    function _get(Map storage map, bytes32 key, string memory errorMessage) internal view returns (bytes32) {
        bytes32 value = map._values[key];
