...
When an FTSO is used for asset/$ price, ex: XRP/$, the fXRP and wFLR holders will have the vote power to submit prices. When discussing weights for the FTSO which gives FLR/$, we could say every asset should be weighted for price calculation since FLR/$ price affects collateral ratio calculations of all fAssets. To avoid extra gas costs for this FTSO, a partial list of fAssets will be defined. The vote power of each price submission will take into account its voter power with every fAsset on the list. The list will include top minted fAssets reflecting the minted $ value. The fAsset weight per address aggregates its vote power from this list. After this vote power aggregation is calculated, the FTSO code will have the exact same algorithm. 
...
[FTSO Manager]: ../../contracts/ftso/implementation/Ftso.sol "FTSO Manager"
