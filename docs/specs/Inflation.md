...
Minting is fulfilled be the [FlareDaemon] by passing on the minting request to the validator at the end of `trigger()` by returning the mint request total from the trigger method. At the end of state transition, the validator will add the requested FLR to the [FlareDaemon] contract balance.

The process concludes with the [FlareDaemon], in the next block, recognizing and transferring added FLR from the validator by calling `receiveMinting` on [Inflation]. The [FlareDaemon] detects arrival of this new FLR by monitoring the contract balance every time `trigger()` is called (every block).
